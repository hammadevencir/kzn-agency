...
/**
 * Resolve the top-up-fee label to display for a stored pricing snapshot,
 * falling back to the standard fee when none was captured (e.g. legacy ad
 * accounts created before pricing snapshots existed). A genuinely stored
 * "0%" (a real perk on some plans) is left as-is — only a missing/blank
 * value falls back.
 * @param {unknown} label
 * @returns {string}
 */
export function topUpFeeLabelOrDefault(label) {
  return typeof label === "string" && label.trim()
    ? label
    : formatTopUpFeePct(DEFAULT_TOP_UP_FEE_PCT);
}
