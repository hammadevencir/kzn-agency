...
const REQUEST_LABELS = {
  ...
  timezone: "Ad account time zone",
  website: "Website link",
  confirmHat: "White-hat advertising confirmation",
  advertiseDetails: "What you advertise",
  supplierName: "Supplier",
  previousProvider: "Previous ad account provider",
  country: "Advertising country",
  bcId: "Business Center (BC) ID",
  accountEmail: "Account management email",
  profitModel: "Profit model",
  firstDeposit: "First deposit amount",
  publicProfileName: "Public Profile Name",
  publicProfileId: "Public Profile ID",
  twitterAccountLink: "Twitter/X account link",
  twitterHandle: "Twitter/X handle",
  pinterestHomepage: "Pinterest homepage link",
};
...
export function mapAdAccountApprovedRow(id, data, topUpDocs = []) {
  ...
  const pricing =
    flow.pricingSnapshot && typeof flow.pricingSnapshot === "object"
      ...
      : {};
  const packageName =
    [flow.displayPlatform, flow.requestTypeLabel, flow.planTier]
      .filter(Boolean)
      .join(" ") ||
    (typeof flow.planName === "string" ? flow.planName : "—");
  const topUpFee = topUpFeeLabelOrDefault(pricing.topUpFee);
  const monthlyFee =
    typeof pricing.monthlyFee === "string" && pricing.monthlyFee.trim()
      ? pricing.monthlyFee
      : "—";
  return {
    firestoreId: id,
