...
import { LOGIN_PATH, ROLE } from '@/lib/auth/constants';
...
function getSettingsContext(role) {
  ...
  return {
    ...
    portal: ROLE.USER,
    loginPath: LOGIN_PATH[ROLE.USER],
  };
}
 ...
 * @param {ReturnType<typeof getSettingsContext>} ctx
 */
async function syncProfileToServer(user, ctx) {
  await withTimeout(user.getIdToken(true), 15000, 'token_refresh');
  await withTimeout(establishSession(user, ctx.portal), 15000, 'session');
  const ctrl = new AbortController();
  const timeoutId = setTimeout(() => ctrl.abort(), 15000);
  try {
    const syncRes = await fetch(ctx.profilePath, {
      method: 'PATCH',
...
export default function Settings({ role = 'admin' }) {
  ...
  const ctx = getSettingsContext(role);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(ctx.profilePath, { credentials: 'include' });
        if (cancelled) return;
        if (res.status === 401) {
          router.replace(ctx.loginPath);
          return;
        }
  ...
  }, [router, ctx.loginPath, ctx.profilePath]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await syncProfileToServer(fresh, ctx);
  ...
  const handleRemoveAvatar = async () => {
    const u = auth.currentUser;
    if (!u || uploadingAvatar) return;
    const hasRemote = photoURL && String(photoURL).trim().length > 0;
    if (!hasRemote) return;
  ...
  const showRemovePhoto = Boolean(photoURL && String(photoURL).trim().length > 0);

  const isUser = role === 'user';
  const profileText = isUser ? 'User Profile' : 'Admin Profile';
